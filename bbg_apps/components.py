...
def generate_layout(graphs, configs):
    ...
    current_graph = configs["current_graph"] if "current_graph" in configs else ""
    ...
    node_freq_type = configs["node_weight"] if "node_weight" in configs else "degree_frequency"
    edge_freq_type = configs["edge_weight"] if "edge_weight" in configs else "npmi"
    nodefreqslider = configs["nodefreqslider"] if "nodefreqslider" in configs else [0, 100000]
    ...
    searchpathto = configs["searchpathto"] if "searchpathto" in configs else None
    searchnodetotraverse = configs["searchnodetotraverse"] if "searchnodetotraverse" in configs else None
    searchpathlimit = configs["searchpathlimit"] if "searchpathlimit" in configs else 10
    searchpathoverlap = configs["searchpathoverlap"] if "searchpathoverlap" in configs else [1]
    ...
    node_range_group = dbc.FormGroup(
        [
            ...
            html.Div(
                [dcc.RangeSlider(id="nodefreqslider_content", min=0, max=100000, value=nodefreqslider)],
                id="nodefreqslider"
            ),
            dbc.Tooltip(
    ...
    [
        ...
        html.Div(
            [dcc.RangeSlider(id="edgefreqslider_content", min=0, max=100000, value=edgefreqslider)],
            id="edgefreqslider"),
        dbc.Tooltip(
    ...
    cluster_group = dbc.InputGroup(
        [
            ...
            dcc.Dropdown(
                id="cluster_type",
                value=cluster_type,
                options=[{'label': val[0], 'value': val[1]} for val in cluster_types],
                style={"width":"100%"})
        ],
        className="mb-1"
    )
    ...
    path_from = dbc.FormGroup([
        ...
        dbc.Col(dcc.Dropdown(
            ...
            options=([{"label": searchpathfrom, "value": searchpathfrom}] if searchpathfrom else [])
        ), width=9)],
        row=True)
    ...
    path_to = dbc.FormGroup([
        ...
        dbc.Col(dcc.Dropdown(
            id="searchpathto",
            options=[{"label": searchpathto, "value": searchpathto}] if searchpathto else [],
            value=searchpathto
        ), width=9)],
        row=True)
    ...
    path_condition = dbc.FormGroup([
        ...
        dbc.FormGroup([
                dbc.Col([
                    ...
                    dcc.Dropdown(id="searchnodetotraverse", value=searchnodetotraverse, options=[{"label": searchnodetotraverse, "value": searchnodetotraverse}] if searchnodetotraverse else [])
                ], width=6),
                dbc.Col([
                    dbc.Label(
    ...
    expand_edge = dbc.FormGroup([
        ...
        dbc.FormGroup([
            ...
            dbc.Col(dbc.Label("N best paths", html_for="expand-edge-n", style={"margin-top": "5pt"}), width=3),
            dbc.Col(
                daq.NumericInput(
                    id="expand-edge-n",
                    min=1,  
