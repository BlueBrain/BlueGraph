...
def save_run(app, initial_port, mode="jupyterlab", debug=False, inline_exceptions=None,
             ascending=True, retry_limit=5):
    if mode not in SUPPORTED_JUPYTER_DASH_MODE:
        raise Exception(
            "Please provide one of the following mode value: " + str(SUPPORTED_JUPYTER_DASH_MODE))
    ...
    for i in range(retry_limit):
        try:
            ...
            if ascending:
                ...
                new_port = port - 1
            print(f"Opening port number {port} failed: {str(ose)}. Trying port number {new_port} ...")
            port = new_port
...
def merge_cyto_elements(elements, nodes_to_merge, new_name=None):            
    ...
    if new_name not in node_dict:
        ...
        removed_nodes[nodes_to_merge[0]] = copy.deepcopy(node_dict[nodes_to_merge[0]])
    ...
    for e_id, el in edge_dict.items():
        if el["data"]["source"] in other_nodes + [target_node] + ([old_target_name] if old_target_name else []):
            neighbour = el["data"]["target"]
            if neighbour not in other_nodes and neighbour != target_node and neighbour != old_target_name:
        ...
        elif el["data"]["target"] in other_nodes + [target_node] + ([old_target_name] if old_target_name else []):
    ...
    for k, v in edge_attrs.items():
        if k not in target_neighbours:
            ...
            edge_id = "{}_{}".format(target_node, k).replace(",", "_").replace(" ", "_")
            ...
            removed_edges[target_neighbours[k]["id"]] = {"data": copy.deepcopy(target_neighbours[k])}
        merge_attrs(target_neighbours[k], v, ATTRS_RESOLVER, attrs_to_ignore=["id", "source", "target", "type"])
